...
class Detector_v4:
    lib = CDLL(hp.modelPath, RTLD_GLOBAL) 
    ...
    def detect_im(self,image, thresh=.5, hier_thresh=.5, nms=.45, debug= False):
        """
...
#         self.free_image(im)
    ...
    def detect_image(self,image, thresh=.5, hier_thresh=.5, nms=.45, debug= False):
        ...
        custom_image = cv2.resize(custom_image,(self.lib.network_width(self.net), self.lib.network_height(self.net)), interpolation = cv2.INTER_LINEAR)
        #import scipy.misc
        ...
        dets = self.get_network_boxes(self.net, image.shape[1], image.shape[0], thresh, hier_thresh, None, 0, pnum, letter_box) # OpenCV
#         dets = self.get_network_boxes(self.net, im.w, im.h, thresh, hier_thresh, None, 0, pnum, letter_box)
    ...
    def detect(self,image, thresh= 0.25, configPath = hp.configurationFile, weightPath = hp.weightFile, metaPath= hp.dataFile, showImage= False, makeImageOnly = False, initOnly= False):
        """
        ...
        if self.net is None:
            self.net = self.load_net_custom(configPath, weightPath, 0, 1) 
             # batch size = 1
        if self.meta is None:
            self.meta = self.load_meta(metaPath)
        if self.altNames is None:
        ...
        detections = self.detect_im(image, thresh)
        if showImage:
            try:
